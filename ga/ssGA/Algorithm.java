...
public class Algorithm {
  ...
  public Algorithm(Problem p, int popsize, int gn, int gl, double pc, double pm) throws Exception {
    ...
    for (int i = 0; i < popsize; i++) pop.set_fitness(i, problem.evaluateStep(pop.get_ith(i)));
    pop.compute_stats();
  }
  ...
  public Individual select_tournament() throws Exception {
    ...
    if (pop.get_ith(p1).get_fitness() > pop.get_ith(p2).get_fitness()) return pop.get_ith(p1);
    else return pop.get_ith(p2);
  }
  ...
  public Individual mutate(Individual p1) {
    ...
    for (int i = 0; i < chrom_length; i++)
      ...
      {
        if (aux_indiv.get_allele(i) == 1) aux_indiv.set_allele(i, (byte) 0);
        else aux_indiv.set_allele(i, (byte) 1);
      }
