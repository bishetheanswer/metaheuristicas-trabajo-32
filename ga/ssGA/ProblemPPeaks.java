...
public class ProblemPPeaks extends Problem {
  ...
  private double MMDP6(Individual indiv) {
    ...
    for (i = 0; i < len; i++) {
      unitation = 0;
      for (j = 0; j < 6; j++) if (indiv.get_allele(index++) == 1) unitation++;
  ...
  private double SUBSET_SUM(Individual indiv) {
    ...
    if (CL != 128) System.out.println("Length mismatch error in Subset sum function.");

    for (i = 0; i < CL; i++) fitness += indiv.get_allele(i) * w[i];
